...
for period = 1 : test_length
    ...
    minimize( 1 )
    ones(1, n) * x == 1;
    gamma_rps + alpha_rps * (d - 1) - 0.25 * eta' * z <= -a;
    for k = 1 : T
        r = R(:, k);
        norm([z(k); 0.5 * (alpha_rps - mu_rps(k))]) <= 0.5 * (alpha_rps + mu_rps(k));
        r' * x + z(k) + gamma_rps >= 2 * alpha_rps;
    end
    x >= 0;
    gamma_rps >= 0;
    alpha_rps >= 0;
    ...
    if d == 0
        ...
        sum(square(pi_opt) ./ eta + eta - 2 * pi_opt) <= d;
        ones(1, T) * pi_opt == 1;
        pi_opt >= 0;
        cvx_end
    ...
    for iteration = 1 : 50
        ...
        for step = 1 : 10
            ...
            cvx_begin quiet
            cvx_solver mosek
            variables x_poj(n, 1) alpha_rps gamma_rps y(T, 1) z(T, 1) mu_rps(T, 1)
            ...
            for k = 1 : T
                r = R(:, k);
                norm([z(k); 0.5 * (alpha_rps - mu_rps(k))]) <= 0.5 * (alpha_rps + mu_rps(k));
                r' * x_poj + z(k) + gamma_rps >= 2 * alpha_rps;
            end
            x_poj >= 0;
            gamma_rps >= 0;
            alpha_rps >= 0;
...
save('../Result/Real_Modified_x2(d=0.1).mat', 'd', 'average_cost', 'cost_array', 'portfolio_classical', 'portfolio_robust', 'relative_cost', 't_array', 'wealth_classical', 'wealth_n', 'wealth_robust');
