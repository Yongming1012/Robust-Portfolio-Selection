...
for period = 1 : test_length
    ...
    minimize( 1 )
    ones(1, n) * x == 1;
    gamma_rps + alpha_rps * d - alpha_rps + eta' * y <= -a;
    for k = 1 : T
        r = R(:, k);
        norm([alpha_rps; 0.5 * (y(k) - alpha_rps - r' * x - gamma_rps)]) <= 0.5 * (y(k) + alpha_rps + r' * x + gamma_rps);
    ...
    if d == 0
        ...
        sum(pi_opt + eta - 2 * sqrt(pi_opt .* eta)) <= d;
        ones(1, T) * pi_opt == 1;
        pi_opt >= 0;
        cvx_end
    ...
    for iteration = 1 : 1000
        ...
        for step = 1 : 10
            ...
            for k = 1 : T
                r = R(:, k);
                norm([alpha_rps; 0.5 * (y(k) - alpha_rps - r' * x_poj - gamma_rps)]) <= 0.5 * (y(k) + alpha_rps + r' * x_poj + gamma_rps);
...
save('../Result/Real_Hellinger(d=0.1).mat', 'd', 'average_cost', 'cost_array', 'portfolio_classical', 'portfolio_robust', 'relative_cost', 't_array', 'wealth_classical', 'wealth_n', 'wealth_robust');
