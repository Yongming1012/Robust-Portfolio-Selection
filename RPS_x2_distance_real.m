...
for period = 1 : test_length
    ...
    minimize( 1 )
    ones(1, n) * x == 1;
    gamma_rps + alpha_rps * (2 + d) - 2 * eta' * y <= -a;
    for k = 1 : T
        r = R(:, k);
        norm([y(k); 0.5 * (r' * x + gamma_rps)]) <= alpha_rps + 0.5 * (r' * x + gamma_rps);
    ...
    if d == 0
        ...
        sum(pi_opt + pow_p(pi_opt, -1) .* eta_2 - 2 * eta) <= d;
        ones(1, T) * pi_opt == 1;
        pi_opt >= 0;
        cvx_end
    ...
    for iteration = 1 : 1000
        ...
        for step = 1 : 10
            ...
            for k = 1 : T
                r = R(:, k);
                norm([y(k); 0.5 * (r' * x_poj + gamma_rps)]) <= alpha_rps + 0.5 * (r' * x_poj + gamma_rps);
...
save('../Result/Real_x2(d=0.1).mat', 'd', 'average_cost', 'cost_array', 'portfolio_classical', 'portfolio_robust', 'relative_cost', 't_array', 'wealth_classical', 'wealth_n', 'wealth_robust');
